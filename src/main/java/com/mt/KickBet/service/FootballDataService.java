...
public class FootballDataService {
    ...
    private LocalDateTime getEndOfTwoWeeks() {
    ...
    public int syncMatches() {
        try {           
            List <String> leagues = Arrays.asList(competitionIds.split(","));
            LocalDateTime endOfTwoWeeks = getEndOfTwoWeeks();
            int savedCount = 0;
            ...
            for (String competitionId : leagues) {
                ...
                try {
                    ...
                    for (FootballApiMatch apiMatch : apiResponse.getMatches()) {
                        ...
                        if ("FINISHED".equals(apiMatch.getStatus()) || "LIVE".equals(apiMatch.getStatus()) || "CANCELLED".equals(apiMatch.getStatus())) continue;

                        LocalDateTime startTime = apiMatch.getStartTime();
                        if (startTime == null)  continue;

                        if (startTime.isAfter(endOfTwoWeeks)) continue;
